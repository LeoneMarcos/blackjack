...
function isValidSignalPayload(
  ...
  if (type === 'offer') {
    ...
    if (!sdpValid) {
      return { valid: false, reason: 'Offer must include valid sdp.' };
    }
    return { valid: true };
  }
  ...
  if (type === 'answer') {
    ...
    const sdpValid =
      ...
      (typeof d.sdp === 'object' &&
        ...
        typeof (d.sdp as Record<string, unknown>).sdp === 'string');
    if (!sdpValid) {
      return { valid: false, reason: 'Answer must include valid sdp.' };
    }
    return { valid: true };
  }
...
export class RoomDO {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    // Reject guest joining a nonexistent room
    ...
    // Reject creating a room if host already exists
    ...
    if (role === 'guest') {
      ...
      // Notify guest that host is present
  ...
  async webSocketMessage(ws: WebSocket, message: string | ArrayBuffer): Promise<void> {
    ...
    // Validate signal payload shape and role direction
    ...
    // Forward signaling payload strictly to the opposite peer
